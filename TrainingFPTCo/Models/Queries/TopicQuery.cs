...
namespace TrainingFPTCo.Models.Queries
{
    public class TopicQuery
    {
            ...
            using (SqlConnection connection = Database.GetSqlConnection())
            {
                ...
                using (SqlDataReader reader = cmd.ExecuteReader())
                {
                    while (reader.Read())
                    {
                        ...
                        topicDetail.ViewAttachFile = reader["AttachFiles"].ToString();
        ...
        )
        {
            ...
            using (SqlConnection connection = Database.GetSqlConnection())
            {
                string sqlQuery = "INSERT INTO [Topics]([Name], [CourseId], [Description], [Documents], [AttachFiles], [PosterTopic], [Status], [CreatedAt]) VALUES(@Name, @CourseId, @Description, @Documents, @AttachFiles, @PosterTopic, @Status, @CreatedAt) SELECT SCOPE_IDENTITY()";
                ...
                cmd.Parameters.AddWithValue("@AttachFiles", attachFilesTopic ?? DBNull.Value.ToString());
        ...
        )
        {
            ...
            using (SqlConnection connection = Database.GetSqlConnection())
            {
                string sqlUpdate = "UPDATE [Topics] SET [Name] = @name, [CourseId] = @Courses, [Description] = @description, [Documents] = @documents, [AttachFiles] = @attachFiles, [PosterTopic] = @posterTopic, [Status] = @status, [UpdatedAt] = @updatedAt WHERE [Id] = @id AND [DeletedAt] IS NULL";
                connection.Open();
