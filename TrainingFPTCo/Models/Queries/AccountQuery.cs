...
namespace TrainingFPTCo.Models.Queries
{
    public class AccountQuery
    {
       ...
       )
        {
            ...
            {
                string sqlQuery = "INSERT INTO [Users] ([RolesId], [Username], [Password], [ExtraCode], [Email], [Phone], [Address], [FullName], [FirstName], [LastName], [Birthday], [Gender], [Education], [ProgramingLanguage], [ToeicScore], [Skills], [IPClient], [CreatedAt]) VALUES (@RolesId, @Username, @Password, @ExtraCode, @Email, @Phone, @Address, @FullName, @FirstName, @LastName, @Birthday, @Gender, @Education, @ProgramingLanguage, @ToeicScore, @Skills, @IPClient, @CreatedAt); SELECT SCOPE_IDENTITY()";
            ...
            using (SqlConnection connection = Database.GetSqlConnection())
            {
                ...
                using (SqlDataReader reader = command.ExecuteReader())
                {
                    while (reader.Read())
                    {
                        ...
                        accountDetail.RolesId = Convert.ToInt32(reader["RolesId"]);
                        // Lấy thông tin chi tiết của Role từ bảng Roles (cần thay đổi tên cột nếu cần)
                        // Ví dụ: accountDetail.Role = GetRoleById(accountDetail.RolesId);
                        ...
                        accountDetail.IPClient = reader["IPClient"].ToString();
        ...
        public Role GetRoleById(int rolesId)
        {
            Role role = null;
            using (SqlConnection connection = Database.GetSqlConnection())
            {
                string sqlQuery = "SELECT * FROM [Roles] WHERE [Id] = @rolesId";
                connection.Open();
                SqlCommand command = new SqlCommand(sqlQuery, connection);
        ...
        public List<AccountDetail> GetAllAccounts(string? searchString, string? filterStatus)
        {
            ...
            using (SqlConnection connection = Database.GetSqlConnection())
            {
                ...
                {
                    sqlQuery = "SELECT * FROM [Users] WHERE ([Username] LIKE @searchString OR [FullName] LIKE @searchString) AND [DeletedAt] IS NULL AND [Status] = @status";
                }
                else
                {
                    sqlQuery = "SELECT * FROM [Users] WHERE [Username] LIKE @searchString OR [FullName] LIKE @searchString AND [DeletedAt] IS NULL";
                }
                ...
                using (SqlDataReader reader = command.ExecuteReader())
                {
                    while (reader.Read())
                    {
                        ...
                        account.Role = GetRoleById(Convert.ToInt32(reader["RolesId"]));
                        account.Id = Convert.ToInt32(reader["Id"]);
                        account.RolesId = Convert.ToInt32(reader["RolesId"]);
                        // Lấy thông tin chi tiết của Role từ bảng Roles (cần thay đổi tên cột nếu cần)
                        // Ví dụ: account.Role = GetRoleById(account.RolesId);
                        account.Role = GetRoleById(account.RolesId);
                        ...
                        // Kiểm tra xem trường Birthday có giá trị null hay không trước khi chuyển đổi
                        ...
                        // Kiểm tra xem trường ToeicScore có giá trị null hay không trước khi chuyển đổi
                        ...
                        account.IPClient = reader["IPClient"].ToString();
...
)
        {
            ...
            using (SqlConnection connection = Database.GetSqlConnection())
            {
                string sqlUpdate = "UPDATE [Users] SET [RolesId] = @rolesId, [Username] = @username, [Password] = @password, [ExtraCode] = @extraCode, [Email] = @email, [Phone] = @phone, [Address] = @address, [FullName] = @fullName, [FirstName] = @firstName, [LastName] = @lastName, [Birthday] = @birthday, [Gender] = @gender, [Education] = @education, [ProgramingLanguage] = @programingLanguage, [ToeicScore] = @toeicScore, [Skills] = @skills, [IPClient] = @ipClient, [UpdatedAt] = @updatedAt WHERE [Id] = @id"; 
                connection.Open();
